...
actor OebbAPIClient {
    ...
    init(storage: StorageServiceProtocol = LocalStorageService.shared, urlSession: URLSession = .shared) {
        ...
        self.authSession = try? storage.load(forKey: StorageKey.oebbAuthSession.rawValue)
    }
    ...
    func searchStations(query: String, count: Int) async throws -> [OebbStation] {
        var components = URLComponents(
            url: baseURL.appendingPathComponent("api/hafas/v1/stations"),
    ...
    func fetchTimetable(
        ...
        let payload = OebbTimetableRequest(
            reverse: false, datetimeDeparture: dateFormatter.string(from: departure),
            filter: .default, passengers: [.default], count: count, debugFilter: .default,
            from: .init(latitude: from.latitude, longitude: from.longitude, name: from.name, number: from.number),
            to: .init(latitude: to.latitude, longitude: to.longitude, name: to.name, number: to.number),
            timeout: [:]
        )
        ...
        return try await performAuthorized(
            ...
            decode: OebbTimetableResponse.self
        )
    }
    ...
    func fetchStationBoard(evaId: Int, directionId: Int? = nil, date: Date = Date()) async throws -> OebbStationBoard {
        ...
        var queryItems = [
            ...
            URLQueryItem(name: "evaId", value: String(evaId)),
            URLQueryItem(name: "boardType", value: "dep"),
            URLQueryItem(name: "time", value: time),
            URLQueryItem(name: "productsFilter", value: "1111111111111111"),
            URLQueryItem(name: "additionalTime", value: "0"),
            URLQueryItem(name: "maxJourneys", value: "50"),
            URLQueryItem(name: "outputMode", value: "tickerDataOnly"),
            URLQueryItem(name: "start", value: "yes"),
            URLQueryItem(name: "selectDate", value: "period"),
            URLQueryItem(name: "dateBegin", value: dateStr),
        ...
        guard let startRange = text.range(of: "="),
              let endRange = text.range(of: ";SLs.", options: .backwards) ?? text.range(of: ";", options: .backwards) else {
            throw GleisError.apiError("Invalid format")
        }
    ...
    private func applyAuth(to request: URLRequest) async throws -> URLRequest {
        ...
        if let cookie = session.cookie, !cookie.isEmpty { req.setValue(
            "ts-cookie=\(cookie)",
    ...
    private func authenticate(userId: String) async throws -> OebbAuthSession {
        var components = URLComponents(
            url: baseURL.appendingPathComponent("api/domain/v3/init"),
            resolvingAgainstBaseURL: false
        )
