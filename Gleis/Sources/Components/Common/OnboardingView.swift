...
struct OnboardingView: View {
    ...
    @ViewBuilder
    private func pageView(for page: OnboardingPageType) -> some View {
        switch page {
        case .welcome:
            PremiumWelcomeView()
        case .interactiveSetup:
        ...
        case .widgetShowcase:
            RealWidgetShowcaseView(pinnedJourney: pinnedJourney)
        case .notifications:
            SmartNotificationView(pinnedJourney: pinnedJourney)
        }
    }
            ...
            VStack(spacing: 40) {
                ...
                ZStack {
                    ...
                    RoundedRectangle(cornerRadius: 55)
                        .fill(colorScheme == .dark ? .black : Color(white: 0.15))
                    ...
                    RoundedRectangle(cornerRadius: 47)
                        .fill(colorScheme == .dark ? Color(.systemBackground) : Color(.systemGroupedBackground))
                        ...
                        VStack(spacing: 14) {
                            ...
                            VStack(spacing: 8) {
                                ...
                                ZStack {
                                    ...
                                    Circle()
                                        .trim(from: 0, to: countdownSeconds / 300.0)
                                        .stroke(urgencyColor, style: StrokeStyle(lineWidth: 6, lineCap: .round))
                                        .frame(width: 120, height: 120)
                                        .rotationEffect(.degrees(-90))
                                        .animation(.linear(duration: 1), value: countdownSeconds)

                                    VStack(spacing: 4) {
                                        Text(formatTime(Int(countdownSeconds)))
                            ...
                            HStack {
                                ...
                                VStack(alignment: .trailing, spacing: 4) {
                                    ...
                                    Text("3")
                                        .font(.system(size: 16, weight: .semibold))
                                        .foregroundStyle(showPlatformUpdate ? .green : .primary)
                                }
                            }
                ...
                VStack(spacing: 12) {
                    ...
                    Text("Live countdowns, platform updates, and smart notifications")
                        .font(.body)
    ...
    private func startPlatformAnimation() {
        Timer.scheduledTimer(withTimeInterval: 8.0, repeats: true) { _ in
            withAnimation(.spring(response: 0.5, dampingFraction: 0.7)) {
                showPlatformUpdate = true
            }

            DispatchQueue.main.asyncAfter(deadline: .now() + 2) {
                withAnimation {
                    showPlatformUpdate = false
                }
            }
        }
    }
...
struct InteractiveJourneySetupView: View {
    ...
    var body: some View {
        ZStack {
            ...
            VStack(spacing: 24) {
                ...
                // Title
                VStack(spacing: 12) {
                    Text("Let's Find Your First Train")
                        .font(.title.weight(.bold))
                        .multilineTextAlignment(.center)

                    Text("Select where you're traveling from and to")
                        .font(.body)
                        .foregroundStyle(.secondary)
                        .multilineTextAlignment(.center)
                        .padding(.horizontal, 40)
                ...
                if usingSampleData {
                    HStack(spacing: 8) {
                        ...
                        Text("Using sample data - check your connection")
                            .font(.caption)
                            .foregroundStyle(.secondary)
                    }
                    .padding(.horizontal)
                ...
                if isLoading {
                    ...
                    VStack(spacing: 12) {
                        if pinnedJourney == nil {
                            Text("Tap the star to save your first journey")
        ...
        .sheet(isPresented: $showOriginPicker) {
            StationPickerWrapper(selectedStation: $originStation)
        }
        .sheet(isPresented: $showDestinationPicker) {
            StationPickerWrapper(selectedStation: $destinationStation)
        }
        .onChange(of: originStation) { _, _ in checkAndFetchConnections() }
    ...
    private func checkAndFetchConnections() {
        ...
        Task {
            await fetchConnections(from: origin, to: destination)
        }
    }
    ...
    private func createSampleConnections(from origin: Station, to destination: Station) -> [TrainConnection] {
        ...
        return [
            TrainConnection(
                ...
                arrivalTime: departure1.addingTimeInterval(1200),
            ...
            TrainConnection(
                ...
                arrivalTime: departure2.addingTimeInterval(1500),
                departureStation: origin,
                arrivalStation: destination,
                platform: "5",
                delay: 2,
...
func withTimeout<R>(seconds: TimeInterval, operation: @escaping () async throws -> R) async throws -> R {
...
struct StationPickerWrapper: View {
    ...
    var body: some View {
        ...
        .onChange(of: searchText) { _, newValue in
            Task { await searchStations(query: newValue) }
        }
        .onAppear {
...
struct SmartNotificationView: View {
    ...
    var notificationText: String {
        if let journey = pinnedJourney {
            ...
            return "\(journey.lineNumber) to \(journey.arrivalStation.name) departing at \(time) from Platform \(platform)"
        } else {
    ...
    var destinationName: String {
        pinnedJourney?.arrivalStation.name ?? "your destination"
    }
        ...
        ZStack {
            ...
            VStack(spacing: 28) {
                ...
                ZStack {
                    ...
                    .offset(y: 60)
                    .blur(radius: showNotification ? 8 : 0)
                    .opacity(showNotification ? 0.3 : 1)
                    ...
                    .padding(.horizontal, 32)
                    .offset(y: showNotification ? 0 : -100)
                ...
                VStack(spacing: 12) {
                    ...
                    Text("Get notified when it's time to leave for \(destinationName)")
                        .font(.body)
                        .foregroundStyle(.secondary)
                        .multilineTextAlignment(.center)
                        .padding(.horizontal, 40)
        ...
        ZStack {
            ...
            VStack(spacing: 32) {
                ...
                VStack(spacing: 12) {
                    ...
                    Text("Stay updated with live countdown widgets")
                        .font(.body)
                        .foregroundStyle(.secondary)
                        .multilineTextAlignment(.center)
                        .padding(.horizontal, 40)
                ...
                if let journey = pinnedJourney {
                    SmallWidgetUIView(
                        ...
                        destination: journey.arrivalStation.name,
                    ...
                    .scaleEffect(animateWidget ? 1.02 : 1.0)
                } else {
                ...
                VStack(alignment: .leading, spacing: 16) {
                    ...
                    HStack(spacing: 12) {
                        ...
                        Text("Search for Gleis and add your widget")
                            .font(.callout)
                    }
                }
                .padding(.horizontal, 40)
                ...
                VStack(alignment: .leading, spacing: 10) {
                    ...
                    FeatureRow(
                        icon: "star.fill",
                        text: "Saved journeys",
                        color: Color(red: 0.0, green: 0.48, blue: 0.85)
                    )
