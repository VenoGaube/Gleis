...
struct SmallWidgetUIView: View {
    ...
    var body: some View {
        VStack(spacing: 6) {
            ...
            if isPinned {
                Text("MY JOURNEY").font(.system(size: 7, weight: .bold)).foregroundStyle(.white).padding(.horizontal, 8)
                    .padding(.vertical, 3).background(Color(red: 0.0, green: 0.48, blue: 0.85), in: Capsule())
            }

            VStack(spacing: 8) {
                HStack(spacing: 10) {
                    Text(lineNumber).font(.system(size: 16, weight: .bold)).foregroundStyle(.white).padding(
                        .horizontal, 10
                    ...
                    VStack(spacing: 2) {
                        Text(countdown).font(.system(size: 24, weight: .bold, design: .rounded)).monospacedDigit()
                            .foregroundStyle(urgencyColor).contentTransition(.numericText())
                        Text("to leave").font(.system(size: 10)).foregroundStyle(.secondary)
                    }
                }
                ...
                HStack(spacing: 5) {
                    ...
                    Text("· Pl. \(platform)").font(.system(size: 12)).foregroundStyle(.secondary)
                }

                HStack(spacing: 4) {
                    Circle().fill(urgencyColor).frame(width: 6, height: 6)
                    Text("Live").font(.system(size: 10)).foregroundStyle(.tertiary)
                }
            }.padding(14)
        }.frame(width: 155, height: 155).background(colorScheme == .dark ? Color(.secondarySystemBackground) : .white)
