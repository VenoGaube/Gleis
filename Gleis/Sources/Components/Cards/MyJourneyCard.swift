...
struct MyJourneyCard: View {
    ...
    var body: some View {
        ...
        .shadow(color: colorScheme == .dark ? .clear : .black.opacity(0.08), radius: 12, y: 6)
    }
        ...
        VStack(spacing: 12) {
            HStack(spacing: 12) {
                ...
                VStack(alignment: .leading, spacing: 4) {
                    ...
                    Text("\(journey.departureStation.name) → \(journey.arrivalStation.name)")
                ...
                if journey.delay > 0 {
                    DelayBadge(minutes: journey.delay)
                }
                ...
                VStack(spacing: 4) {
                    GeometryReader { geo in
                        ZStack(alignment: .leading) {
                            ...
                            RoundedRectangle(cornerRadius: 4)
                                ...
                                .frame(width: geo.size.width * journeyProgress)
                        }
                    }
                    .frame(height: 6)
                    ...
                    HStack {
                        Text(journey.departureTime, style: .time)
                            ...
                            .foregroundStyle(.secondary)
                        Spacer()
                        Text("Arriving \(journey.arrivalTime, style: .time)")
                            .font(.caption2.weight(.medium))
            ...
            Button {
                ...
                HStack(spacing: 4) {
                    Text(isExpanded ? "Hide route" : "Show route")
        ...
        VStack(spacing: 0) {
            ...
            ForEach(Array(legs.enumerated()), id: \.element.id) { legIndex, leg in
                ...
                JourneyLegSection(
                    ...
                    isLastLeg: legIndex == legs.count - 1,
                ...
                if legIndex < legs.count - 1 {
                    ...
                    JourneyTransferRow(
                        ...
                        nextLineNumber: nextLeg.lineNumber,
                        nextPlatform: nextLeg.platform,
    ...
    private func createFallbackLeg() -> ConnectionLeg {
        ConnectionLeg(
            from: journey.departureStation,
            to: journey.arrivalStation,
        ...
        VStack(spacing: 0) {
            ...
            if isFirstLeg || departureStation != nil {
                JourneyStopRow(
                    ...
                    delay: leg.delayMinutes,
                    lineNumber: leg.lineNumber,
                    isEndpoint: true,
                    isPassed: leg.departureTime.map { now >= $0 } ?? false,
            ...
            if !leg.isWalking {
                ForEach(leg.intermediateStops) { stop in
                    JourneyStopRow(
                        ...
                        lineNumber: nil,
                        isEndpoint: false,
                        isPassed: stop.arrivalTime.map { now >= $0 } ?? false,
            ...
            if isLastLeg {
                JourneyStopRow(
                    ...
                    lineNumber: nil,
                    isEndpoint: true,
                    isPassed: leg.arrivalTime.map { now >= $0 } ?? false,
                    showTopConnector: true,
...
private struct JourneyStopRow: View {
    ...
    var body: some View {
        HStack(alignment: .center, spacing: 12) {
            ...
            VStack(spacing: 0) {
                Rectangle()
                    .fill(showTopConnector ? (isPassed ? Color.accentColor : Color.secondary.opacity(0.3)) : .clear)
                    .frame(width: 2, height: isEndpoint ? 8 : 10)

                Circle()
                    .fill(isPassed ? Color
                        .accentColor : (colorScheme == .dark ? Color(.systemGray4) : Color(.systemGray5)))
                    .frame(width: isEndpoint ? 12 : 6, height: isEndpoint ? 12 : 6)
                    .overlay(
                        Circle()
                            .stroke(
                                isPassed ? Color.accentColor : Color.secondary.opacity(0.5),
                                lineWidth: isEndpoint ? 2 : 1
                            )
                    )

                Rectangle()
                    .fill(showBottomConnector ? (isPassed ? Color.accentColor : Color.secondary.opacity(0.3)) : .clear)
                    .frame(width: 2, height: isEndpoint ? 8 : 10)
            }
            .frame(width: 20)
            ...
            VStack(alignment: .leading, spacing: 1) {
                Text(name)
                    .font(isEndpoint ? .subheadline.weight(.semibold) : .caption)
                ...
                if let time {
                    HStack(spacing: 4) {
                        Text(time, style: .time)
                            .font(isEndpoint ? .caption.monospacedDigit().weight(.medium) : .caption2.monospacedDigit())
                            .foregroundStyle(isPassed ? .secondary : .primary)
...
private struct JourneyTransferRow: View {
    ...
    var body: some View {
        HStack(alignment: .center, spacing: 12) {
            ...
            VStack(spacing: 0) {
                Rectangle()
                    .fill(isPassed ? Color.accentColor : Color.secondary.opacity(0.3))
                    .frame(width: 2, height: 8)

                ZStack {
                    Circle()
                        .stroke(isTightTransfer ? Color.red : Color.orange, lineWidth: 2)
            ...
            VStack(alignment: .leading, spacing: 4) {
                HStack(spacing: 6) {
                    ...
                    Text("Transfer at \(stationName)")
                        .font(.caption.weight(.medium))
                        .foregroundStyle(isTightTransfer ? .red : (isPassed ? .secondary : .primary))
                        .lineLimit(1)

                    Spacer()

                    if let transferMinutes {
                        Text("\(transferMinutes) min")
                            .font(.caption.weight(isTightTransfer ? .semibold : .regular))
                ...
                HStack(spacing: 6) {
                    ...
                    Text(nextLineNumber)
                        ...
                        .padding(.vertical, 2)
                        .background(Color.lineColor(for: nextLineNumber), in: RoundedRectangle(cornerRadius: 3))
            ...
            .background(
                RoundedRectangle(cornerRadius: 8)
                    .fill((isTightTransfer ? Color.red : Color.orange).opacity(colorScheme == .dark ? 0.15 : 0.1))
            )
        }
        .padding(.vertical, 4)
    }
                    ...
                    ConnectionLeg(
                        ...
                        intermediateStops: [
                            ...
                            IntermediateStop(
                                id: "s4",
                                name: "Mürzzuschlag",
                                arrivalTime: Date().addingTimeInterval(4800),
                                departureTime: nil,
                                arrivalDelay: nil,
                                departureDelay: nil,
                                platform: "1"
                            ...
                            IntermediateStop(
                                ...
                                arrivalTime: Date().addingTimeInterval(6000),
                                departureTime: nil,
                                arrivalDelay: nil,
                                departureDelay: nil,
                                platform: "3"
